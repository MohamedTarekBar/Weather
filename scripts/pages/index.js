...
$ ('.searchTools li').click (function (e) {
  ...
  if (e.delegateTarget.classList.contains ('searchCity')) {
    ...
    $ ('.mapContainer').animate ({opacity: 0}, 0, function () {
    ...
    if ($ ('.searchContainer').hasClass ('d-block')) {
      ...
      $ ('.searchContainer').animate ({opacity: 0}, 0, function () {
...
function userWeatherData (data) {
  ...
  for (let day = 0; day < days.length; day++) {
    ...
    <div class="body">
    <div class='my-2'>
      <p class='p-0 m-0'>Max Temp</p>
      <div>${maxtemp_c}</div>
      <div>${maxtemp_f}</div>
    </div>
    <div class='my-2'>
      <p class='p-0 m-0'>Min Temp</p>
      <div>${mintemp_c}</div>
      <div>${mintemp_f}</div>
    </div>
    </div>
  ...
  navigator.geolocation.getCurrentPosition (
    function (pos) {
      ...
      mapboxgl.accessToken =
        'pk.eyJ1IjoibXVobWQwMSIsImEiOiJjbDlhZTAzcm4wOG9tNDVsZXEyOGxpOXA4In0.TCd3kq4jyv-VCD5bcBuVVw';
    ...
    {enapleHighAccuracy: true, timeout: 5000}
  );
}
